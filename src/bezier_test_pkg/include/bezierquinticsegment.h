...
#ifndef BEZIERQUINTICSEGMENT_H
#define BEZIERQUINTICSEGMENT_H

#include<vector>
#include<math.h>
#include<iostream>

class BezierQuinticSegment
{
private:
    ...
    double tj_=0, ta1_=0, ta2_=0, tv1_=0, tv2_=0, tv3_=0;
//    double max_achieved_jrk=0, max_achieved_acc=0, max_achieved_vel=0;

public:
  BezierQuinticSegment();
  BezierQuinticSegment( double start_time, double duration, const std::vector<double> &start_state, const std::vector<double> &end_state ) {
  ...
  static int quad_eq_real_root (const double &a, const double &b, const double &c, double &r1, double &r2);
  static int cubic_eq_real_root (double a, double b, double c, double d, std::vector<double> &roots);
  void compute_maxmin_times();
  void check_maxmin_times_interval();
