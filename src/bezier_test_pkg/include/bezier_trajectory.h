...
private:
    ...
    std::vector<double> positions_, velocities_, accelerations_, jerks_,  durations_,times_, stp_pts_idx_;
    int n_points_=0;
public:
    bezier_trajectory();
...
public:
    ...
    bezier_robot_trajectory(trajectory_msgs::JointTrajectory msg){ // }, double t_start){
        init_from_trajectory_msg( msg );//, t_start);
    }
    ...
    void init_from_trajectory_msg( trajectory_msgs::JointTrajectory msg);// , double t_start);
...
//    std::vector<trajectory_msgs::JointTrajectoryPoint> get_updated_posints();
