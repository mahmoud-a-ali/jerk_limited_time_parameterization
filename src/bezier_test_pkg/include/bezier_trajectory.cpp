/**
\file   bezier_trajectory.cpp
\brief  definition of both bezier_trajectory and bezier_robot_trajectory Classes
    ...
    if ( !waypoints_durations.empty() ){
        ...
        for (int i=0; i< n_points_-1; i++){
            if(durations_[i] < 0.01 ){
                durations_[i] = 0.01;
//                std::cout<<"Warn: segs "<<i << " has duration equal to zero, it will be initialized with 0.01 as min" << std::endl;
            }
        }
    }
    ...
    set_inflection_pts_vel_to_zero();
...
void bezier_trajectory::set_inflection_pts_vel_to_zero(){
    // stop point at which trajectory reverse direction
      for (int pt=1; pt<n_points_-1; pt++) {
          if( (  (positions_[pt+1] - positions_[pt])>= 0 && (positions_[pt] - positions_[pt-1])>= 0  ) || ( (positions_[pt+1] - positions_[pt])<= 0 && (positions_[pt] - positions_[pt-1])<= 0 ) )
              continue;
          stp_pts_idx_.push_back( pt );
      }

      //reset vel, acc to zero for these points
      for (int pt=0; pt< stp_pts_idx_.size(); pt++) {
//          std::cout<<">>> reset vel and acc for inflection poit  "<< stp_pts_idx_[pt] << std::endl;
          velocities_[ stp_pts_idx_[pt] ] =0 ;
          accelerations_ [ stp_pts_idx_[pt] ] =0;
      }
...
void bezier_robot_trajectory::init_from_trajectory_msg( trajectory_msgs::JointTrajectory msg){ //}, double t_start ){
    std::cout<<">>>> init_bezier_robot_trajectory_from_trajectory_msg: "<<std::endl;
...
//    init(positions_, velocities_, accelerations_, durations_[0], t_start_);
    ...
    // add checks for trajectory points sizes (pos, vel, acc)
    // extract Positions, Velocities ,Accelerations, Times from that trajectory and store them in std::vectors
    ...
    for(int jt=0; jt<n_jts; jt++){
        for(int pt=0; pt<n_pts; pt++){
            ...
            Vel_jt_wpt[jt].push_back( 1*msg.points[pt].velocities[jt] );
            ACC_jt_wpt[jt].push_back( 1*msg.points[pt].accelerations[jt] );
//            Times_jt_wpt[jt].push_back(0);
    ...
    std::cout<<">>>> init_from_trajectory_msg: \n old times: "<<std::endl;
    for (int i=0; i< Times_jt_wpt[0].size(); i++) {
        std::cout<< Times_jt_wpt[0][i] << std::endl;
    }

    init(Pos_jt_wpt , Vel_jt_wpt, ACC_jt_wpt, required_duration, t_start );
...
//std::vector<trajectory_msgs::JointTrajectoryPoint>  bezier_robot_trajectory::get_updated_posints(){
//    std::vector<trajectory_msgs::JointTrajectoryPoint> points;
//    trajectory_msgs::JointTrajectoryPoint point;
//    for (int pt = 0; pt < n_points_; ++pt) {
//        for (int jt=0; jt<n_joints_; jt++) {
//            point.positions[jt]= positions_[jt][pt];
//            point.velocities[jt] = velocities_[jt][pt];
//            point.accelerations[jt] = accelerations_[jt][pt];
//        }
...
void bezier_robot_trajectory::update_jointTrajectory_msg(trajectory_msgs::JointTrajectory &msg){
    if(max_pos_ <= 0 || max_vel_ <= 0 || max_acc_ <= 0|| max_jrk_ <= 0)
        throw(std::invalid_argument("segment can't be constructed: absolute limits should be positive"));
    std::vector<double> new_times = update_waypoints_times();
    for (int pt = 0; pt < n_points_; ++pt) {
        for (int jt=0; jt<n_joints_; jt++) {
            msg.points[pt].positions[jt]= positions_[jt][pt];
            msg.points[pt].velocities[jt] = velocities_[jt][pt];
            msg.points[pt].accelerations[jt] = accelerations_[jt][pt];
            msg.points[pt].time_from_start = ros::Duration(new_times[pt]);
        }
    }
}
